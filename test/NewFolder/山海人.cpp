...
#include <ctime>
void drawSurface(Mat<float> z, double xs, double xe, double ys, double ye, GraphicsND* G) {
	Mat<float> p(3), pl(3), pu(3), FaceVec, tmp, light(3); light.fill(1).normalized();
...
int main() {
	...
	//旋转视角
	Mat<float> t1(3), t2(3), t3(3), map(N, N), t = map;
	...
	//人像
	Mat<float> faceVec, p1, p2, p3, t0(3), light(3); light.fill(1).normalized();
