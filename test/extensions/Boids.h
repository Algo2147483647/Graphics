...
class Boids {
public:
	struct Boids_cell { Mat<float> r{ 3,1 }, v{ 3,1 }, a{ 3,1 }; };
	...
	Mat<float>* obstacleAvoidance_direction;
	/*********************************[ 函数 ]*********************************/
	/*--------------------------------[ play 运行 ]--------------------------------*/
	...
	void rule(Boids_cell cell[], int n, int index) {
		Mat<float> distance(3);
		Mat<float> avoidDirection(3), groupVelocity(3), groupCenter(3), temp;
		int groupNum = 0;
		for (int i = 0; i < n; i++) {
			if (i == index)continue;
			// 能见范围
	...
	void obstacleAvoidance(Boids_cell& cell) {
		static Mat<float>* obstacleAvoidance_direction;
		static bool o = true;
		if (o) o = false, obstacleAvoidance_direction = Geometry::getSphereFibonacciPoint(200);
		// 确定坐标变换矩阵
		Mat<float> rorateAxis(3), rotateMat, tmp(3);
		GraphicsND::rotate(
	...
	bool obstacleFunction(Mat<float>& point) {
		for (int i = 0; i < 3; i++) {
	...
	void Boids_main() {
		// set Graphics3D
		Mat<float> Axis(3); Axis[0] = 0.7; Axis[1] = 0.4; Axis[2] = 0.59;
		Mat<float> center(3); center[0] = 500; center[1] = 500; center[2] = 500;
		double theta = 1;
		// 
		Mat<float> zero(3), CuboidL(3), CuboidR(3);
