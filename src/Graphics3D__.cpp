...
Mat<float> GraphicsND::TransformMat;											//变换矩阵
...
void GraphicsND::value2pix(double x0, double y0, double z0, int& x, int& y, int& z) {
	Mat<float> point(TransformMat.rows); 
...
void GraphicsND::value2pix(Mat<float>& p0, Mat<int>& pAns) {
	pAns.zero(p0.rows);
	static Mat<float> point; 
...
/*--------------------------------[ 设置坐标范围 ]--------------------------------*/
void GraphicsND::setAxisLim(Mat<float>& pMin, Mat<float>& pMax) {
	Mat<float> redio, tmp;
	redio.sub(pMax, pMin);
...
/*--------------------------------[ 着色器函数例子 ]--------------------------------*/
unsigned int GraphicsND::FaceColorF_1(Mat<float>& p1, Mat<float>& p2, Mat<float>& p3) {
	static Mat<float> t1, t2, faceVec, light(3); light = 1 / sqrt(3);
...
unsigned int GraphicsND::FaceColorF_2(Mat<float>& p1, Mat<float>& p2, Mat<float>& p3) {
	return FaceColor;
}
/*#############################################################################
...
void GraphicsND::drawPoint(Mat<float>& p0) {
	Mat<int> p;
	value2pix(p0, p);
	setPix(p);
}
...
void GraphicsND::drawLine(Mat<float>& sp0, Mat<float>& ep0) {
	Mat<int> sp, ep;
	value2pix(sp0, sp); 
	value2pix(ep0, ep);
...
*                    画折线
******************************************************************************/
void GraphicsND::drawPolyline(Mat<float> *p, int n, bool close) {
...
*                    画Bezier曲线
******************************************************************************/
void GraphicsND::drawBezierLine(Mat<float> p[], int n) {
	/*
	int N = 1000;					//#待优化
	FP64 C[50];
	for (int i = 0; i < n; i++)
...
*	[过程]:
		...
		[3] 类似直线算法, 以 x 方向为基准, 跟踪以 p_x_min 为顶点两条边
			...
			[5] 画线
******************************************************************************/
void GraphicsND::drawTriangle(Mat<float>& p1, Mat<float>& p2, Mat<float>& p3) {
	if (FACE) {
		//[1]
		static Mat<int> pt[3];
		value2pix(p1, pt[0]); 
...
/*--------------------------------[ 画三角形集 ]--------------------------------*/
void GraphicsND::drawTriangleSet(Mat<float>& p1, Mat<float>& p2, Mat<float>& p3) {
	Mat<float> pt1(p1.rows), 
		  pt2(p2.rows), 
		  pt3(p3.rows), 
		  fvt(p1.rows),
		light(p1.rows),tmp;
...
void GraphicsND::drawTriangleSet(Mat<float>& p1, Mat<float>& p2, Mat<float>& p3, Mat<float>& FaceVec) {
	Mat<float> pt1(p1	 .rows), 
		  pt2(p2	 .rows), 
		  pt3(p3	 .rows), 
		  fvt(FaceVec.rows),
...
/*--------------------------------[ 画矩形 ]--------------------------------*/
void GraphicsND::drawRectangle(Mat<float>& sp, Mat<float>& ep, Mat<float>* direct) {
	if (direct == NULL) {
		Mat<float> pt = sp;
		if(FACE)
...
/*--------------------------------[ 画四边形 ]--------------------------------*/
void GraphicsND::drawQuadrangle(Mat<float>& p1, Mat<float>& p2, Mat<float>& p3, Mat<float>& p4) {
	if (FACE) {
...
-----------------------------------------------------------------------------*/
void GraphicsND::drawPolygon(Mat<float> p[], int n) { 
	if (FACE) 
...
void GraphicsND::drawSector(Mat<float>& center, double r, double angleSt, double angleEd, double delta, Mat<float>* direct) {
	if (direct == NULL) {
...
*	[约束方程]:
	...
	[推导]:
		...
		Φ = - arcsin(C1 / sqrt(C1² + C²))
**-----------------------------------------------------------------------*/
void GraphicsND::drawEllipse(Mat<float>& center, double rx, double ry, Mat<float>* direct) {
}
/*--------------------------------[ 画曲面 ]--------------------------------*/
void GraphicsND::drawSurface(Mat<float>& z, double xs, double xe, double ys, double ye, Mat<float>* direct) {
	Mat<float> p(3), pl(3), pu(3), plu(3); Mat<float> FaceVec, tmp, light(3); light = 1 / sqrt(3);
...
/*--------------------------------[ 画四面体 ]--------------------------------*/
void GraphicsND::drawTetrahedron(Mat<float>& p1, Mat<float>& p2, Mat<float>& p3, Mat<float>& p4) {
	if (FACE) {
...
**------------------------------------------------------------------------*/
void GraphicsND::drawCuboid(Mat<float>& pMin, Mat<float>& pMax) {
	Mat<float> pMinTmp[3], pMaxTmp[3];
	for (int i = 0; i < 3; i++) {
...
**------------------------------------------------------------------------*/
void GraphicsND::drawFrustum(Mat<float>& st, Mat<float>& ed, double Rst, double Red, double delta) {
	// 计算 Rotate Matrix
	Mat<float> direction, rotateAxis, rotateMat, zAxis(3), tmp; zAxis = { 0, 0, 1 };
	direction.sub(ed, st);
...
/*--------------------------------[ 画圆柱 ]--------------------------------*/
void GraphicsND::drawCylinder(Mat<float>& st, Mat<float>& ed, double r, double delta) {
...
*	[流程]:
		...
		[2] 画经度线
**-----------------------------------------------------------------------*/
void GraphicsND::drawSphere(Mat<float>& center, double r, 
	double thetaSt, double thetaEd, double phiSt, double phiEd, double dAngle
) {
	Mat<float> point(3), pointU(3), pointL(3), pointUL(3);
...
**---------------------------------------------------------------------------------------------------------*/
void GraphicsND::drawSphere2(Mat<float>& center, double r, int n) {
	// 均匀球面点
	Mat<float> point(3);
...
*	[流程]:
		...
		[2] 画经度线
**-----------------------------------------------------------------------*/
void GraphicsND::drawEllipsoid(Mat<float>& center, Mat<float>& r) {
	const int delta = 5;
	Mat<float> point(3);
...
*                    画平移体
******************************************************************************/
void GraphicsND::drawPipe(Mat<float>& st, Mat<float>& ed, double Rst, double Red, int delta) {
	if (Red == -1) Red = Rst;
	// 计算 Rotate Matrix
	Mat<float> direction, rotateAxis, rotateMat, zAxis(3), tmp; zAxis = { 0, 0, 1 };
	direction.sub(ed, st);
...
void GraphicsND::drawPipe(Mat<float>& st, Mat<float>& ed, double R, int delta) {
	drawPipe(st, ed, R, R, delta);
}
void GraphicsND::drawPipe(Mat<float>* p, int N, double R, int delta) {
	for (int i = 0; i < N - 1; i++) drawPipe(p[i], p[i + 1], R, R, delta);
}
void GraphicsND::drawPipe(Mat<float>& path, double R, int delta) {
	Mat<float> p1, p2; path.getCol(0, p1); p2 = p1;
	for (int i = 0; i < path.cols; i++, p2 = p1) drawPipe(path.getCol(i, p1), p2, R, R, delta);
}
void GraphicsND::drawPipe(Mat<float>& st, Mat<float>& ed, Mat<float>& f) {
	// 计算 Rotate Matrix
	Mat<float> direction, rotateAxis, rotateMat, zAxis(3), tmp; zAxis = { 0, 0, 1 };
	direction.sub(ed, st);
	if (direction[0] != 0 || direction[1] != 0) {
	...
	// 画圆台
	Mat<float> stPoint, edPoint, preStPoint, preEdPoint; tmp.alloc(3);
...
void GraphicsND::drawPipe(Mat<float>& path, Mat<float>& f) {
	Mat<float> p1, p2; path.getCol(0, p1); p2 = p1;
...
*                    画旋转体
******************************************************************************/
void GraphicsND::drawRotator(Mat<float>& zero, Mat<float>& axis, Mat<float>& f, int delta, double st, double ed) {
	//Rotate f
	Mat<float> p1(3), p2(3), p3(3), p4(3), ft = f, RotateMat, preRotateMat, RotateMat0, RotateMatTmp;
	Mat<float> rotateAxis, fAxis(3), tmp; fAxis = { 0, 1, 0 };
...
*                    画阶梯
*****************************************************************************
void GraphicsND::drawStairs(Mat<float>& zero, double Length, double Width, double Height, int StairsNum) {
	Mat<float> p1(3), p2(3);
...
*	[流程]:
		[1] 以二进制顺序遍历所有顶点
			[2] 连接该点和所有比该点编码多1的点
******************************************************************************/
void GraphicsND::drawSuperCuboid(Mat<float>& pMin, Mat<float>& pMax) {
...
******************************************************************************/
void GraphicsND::drawSuperSphere(Mat<float>& center, double r) {
	unsigned int Dim = center.rows, maxCode = 0, times = 1, cur;
	double delta = 0.1, tmp;
	Mat<float> point(Dim), tmpMat;
...
void GraphicsND::draw4DSphere(Mat<float>& center, double r) {
...
---------------------------------------------------------------------------*/
void GraphicsND::drawGrid(Mat<float>& delta, Mat<float>& max, Mat<float>& min) {
	int times = 1, cur;
	for (int dim = 0; dim < min.rows; dim++) times *= (max[dim] - min[dim]) / delta[dim] + 1;
	Mat<float> point(min), st, ed; point[0] -= delta[0];
	for (int i = 0; i < times; i++) {
		//[1]
...
/*--------------------------------[ 画等高线 ]--------------------------------*/
void GraphicsND::contour(Mat<float>& map, const int N) {
	int x_step[] = { 1,0,1 }, 
		y_step[] = { 0,1,1 };
	double max = map.max(),
...
void GraphicsND::contour(Mat<float>& map) {
	double min = map.min(),
		 delta = map.max() - min;
	for (int i = 0; i < map.size(); i++)
		g.setPoint(map.i2x(i), map.i2y(i), colorlist((map[i] - min) / delta, 1));
}
void GraphicsND::contour(Mat<float>& mapX, Mat<float>& mapY, Mat<float>& mapZ) {
	double 
...
******************************************************************************/
// 平移
Mat<float>& GraphicsND::translate(Mat<float>& delta, Mat<float>& transMat) {
	Mat<float> translateMat; translateMat.E(transMat.rows);
	for (int i = 0; i < delta.rows; i++) translateMat(i + 1, 0) = delta[i];
	return transMat.mul(translateMat, transMat);
}
// 旋转
Mat<float>& GraphicsND::rotate(Mat<float>& theta, Mat<float>& center, Mat<float>& transMat) {
	Mat<float> tmp, rotateMat(transMat.rows - 1, transMat.cols - 1);
...
// 3D·四元数
Mat<float>& GraphicsND::rotate(Mat<float>& rotateAxis, double theta, Mat<float>& center, Mat<float>& transMat) {
...
// 缩放
Mat<float>& GraphicsND::scale(Mat<float>& ratio, Mat<float>& center, Mat<float>& transMat) {
	Mat<float> tmp;
	translate(center.negative(tmp), transMat);
	Mat<float> scaleMat(transMat.rows - 1, transMat.cols - 1);
	Matrix::scale(ratio, scaleMat);
	tmp.E(transMat.rows).setBlock(scaleMat, 1, 1);
	return translate(center, transMat.mul(tmp, transMat));
}
// 镜像
Mat<float>& GraphicsND::reflect(Mat<float>& e, Mat<float>& center, Mat<float>& transMat) {
	Mat<float> tmp;
	translate(center.negative(tmp), transMat);
	Mat<float> scaleMat(transMat.rows - 1, transMat.cols - 1);
...
// 透视投影
Mat<float>& GraphicsND::perspect(Mat<float>& e, Mat<float>& center, Mat<float>& transMat) {
...
void GraphicsND::interactive() {
	int ch;
	static double v = 1;
	static Mat<float> delta(3), zero(3);
	if (_kbhit()) {
