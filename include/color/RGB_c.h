 ...
 * methods
 */
RGB RGB_create(INT8U r, INT8U g, INT8U b) {
    RGB rgb;
    rgb.R = r;
    rgb.G = g;
    rgb.B = b;
...
RGB* RGB_assign(RGB* a, const RGB* b) {
    a->R = b->R;
    a->G = b->G;
    a->B = b->B;
    return a;
...
RGB* RGB_assignFromARGB(RGB* a, ARGB* b) {
    a->R = *b >> 16;
    a->G = *b >> 8;
    a->B = *b;
    return a;
...
RGB* RGB_multiply(RGB* a, double b) {
    a->R *= b;
    a->G *= b;
    a->B *= b;
    return a;
...
INT8U* RGB_getChannel(RGB* a, int index) {
    switch (index) {
    case 0:
        return &(a->R);
    case 1:
...
ARGB RGB_toARGB(RGB* a) {
    return ((ARGB)(a->B) << 0) |
           ((ARGB)(a->G) << 8) |
           ((ARGB)(a->R) << 16);
}

ARGB RGB_mul(ARGB a, double r) {
    return ((ARGB)((a & 0x0000FF) * r) & 0x0000FF) |
           ((ARGB)((a & 0x00FF00) * r) & 0x00FF00) |
           ((ARGB)((a & 0xFF0000) * r) & 0xFF0000);
}

ARGB RGB_alpha(ARGB a, ARGB b, double r) {
    return ((ARGB)((a & 0x0000FF) * r + (b & 0x0000FF) * (1 - r)) & 0x0000FF) |
